...
func TestConnection_temp(t *testing.T) {
	err := initLogWithFile("connection_map.log")
	if err != nil {
		fmt.Println(err)
		return
	...
	connection, cerr := NewConnection("acj;map;config=[200,4]")
	if !assert.NoError(t, cerr) {
		return
	}
	defer connection.Close()

	request, rerr := connection.CreateMapReadRequest("Jobs")
	if !assert.NoError(t, rerr) {
		fmt.Println("Error create request", rerr)
		return
	}
	...
	var result *Response
	fmt.Println("Read logigcal data:")
	result, err = request.SearchAndOrder("RecordType=J", "Name")
	if !assert.NoError(t, err) {
		return
	}
	result.DumpValues()
	if assert.Equal(t, 1, len(result.Values)) {
		v, verr := result.Values[0].SearchValue("PV")
		if !assert.NoError(t, verr) {
			return
		}
		assert.Equal(t, "Покупатели", v.String())
